...
class OrderManager extends AbstractBaseManager
{
     ...
     * @return void
     */
    public function createOrderFormCartBySessionId(string $sessionId, User $user)
    {
        $cart = $this->cartManager->getRepository()->findOneBy(['sessionId' => $sessionId]);
        if ($cart) {
            $this->createOrderFormCart($cart, $user);
        }
    }

    public function createOrderFormCart(Cart $cart, User $user)
    {
        $order = new Order();
        $order->setOwner($user);
        $order->setStatus(OrderStaticStorage::ORDER_STATIC_CREATED);
        $orderTotalPrice = 0;
